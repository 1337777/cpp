...
(*    | [ H : and _ _ |- _ ] => destruct H (* why wait for intuition *)
    | [ H : prod _ _ |- _ ] => destruct H (* why wait for intuition *) *)
(*    | [ H := _ : and _ _ |- _ ] => destruct H
    | [ H := _ : prod _ _ |- _ ] => destruct H *)
                ...
                match type of T with
                  ...
                  | _ =>
                    ...
                    let i := fresh "i" in (pose (i := e);
                      ...
                      match T with
                          | sig _ => let Hset := fresh "Hset" in let Hset1 := fresh Hset "_1" in let Hset2 := fresh Hset "_2" in (assert (Hset := e); destruct Hset as [Hset_1 Hset_2])
                          | sigT _ => let Hset := fresh "Hset" in let Hset1 := fresh Hset "_1" in let Hset2 := fresh Hset "_2" in (assert (Hset := e); destruct Hset as [Hset_1 Hset_2])
                          | prod _ _ => let Hset := fresh "Hset" in let Hset1 := fresh Hset "_1" in let Hset2 := fresh Hset "_2" in (assert (Hset := e); destruct Hset as [Hset_1 Hset_2])
                          | sum _ _ => let Hset := fresh "Hset" in let Hset1 := fresh Hset "_1" in let Hset2 := fresh Hset "_2" in (assert (Hset := e); destruct Hset as [Hset_1 | Hset_2])
                          | _ => idtac
                      end  *))
                end
          end
    ...
    (sintuition; rewriter;
      match lemmas with
        ...
        | _ =>
          ...
          (** we do not distinguish [Prop] as [inster] do, therefore we lack [lossyIntuition :=] *)

          (*intuition ( auto with * ; 
                      try solve [ progress (repeat match goal with
                                           | [ J := _ : sig _ |- _ ] => 
                                           | [ J := _ : sigT _ |- _ ] => let Hset := fresh "Hset" in (generalize J; intros Hset; destruct Hset )
                                           | [ J := _ : prod _ _ |- _ ] => let Hset := fresh "Hset" in (generalize J; intros Hset; destruct Hset )
                                           | [ J := _ : sum _ _ |- _ ] => let Hset := fresh "Hset" in (generalize J; intros Hset; destruct Hset )
                                         end); crush' lemmas invOne ] ); *) 
(** DO NOT WORK        (** allow infinite nesting but with timeout *)
          try (timeout 1 (intuition (crush' lemmas invOne))); *)
          un_done
      end;
